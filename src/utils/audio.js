  ...
  const promise = (async () => {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (segment.text && segment.text.trim()) {
        await speak(segment.text, enabled, segment.style);
      }
...
let audioCtx = null;
let globalAudioEnabled = true; // tracks the current mute state for sound effects

/** Call this whenever audioEnabled changes so sounds respect the mute toggle. */
...
export function playTone(frequency, duration = 200) {
  if (!globalAudioEnabled) return; // respect mute
  try {
    const ctx = getCtx();
